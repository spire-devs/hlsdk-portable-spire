...
void CBaseEntity::FireBullets( ULONG cShots, Vector vecSrc, Vector vecDirShooting, Vector vecSpread, float flDistance, int iBulletType, int iTracerFreq, int iDamage, entvars_t *pevAttacker )
{
	...
	for( ULONG iShot = 1; iShot <= cShots; iShot++ )
	{
		...
		if( iTracerFreq != 0 && ( tracerCount++ % iTracerFreq ) == 0 )
		{
			...
			case BULLET_MONSTER_SMALLROUND:
			case BULLET_MONSTER_MEDIUMROUND:
			case BULLET_MONSTER_LARGEROUND:
			default:
		...
		if( tr.flFraction != 1.0f )
		{
			...
			case BULLET_MONSTER_SMALLROUND:
				pEntity->TraceAttack( pevAttacker, gSkillData.monDmgSmallRound, vecDir, &tr, DMG_BULLET );

				TEXTURETYPE_PlaySound( &tr, vecSrc, vecEnd, iBulletType );
				DecalGunshot( &tr, iBulletType );
				break;
			case BULLET_MONSTER_MEDIUMROUND:
				pEntity->TraceAttack( pevAttacker, gSkillData.monDmgMediumRound, vecDir, &tr, DMG_BULLET );

				TEXTURETYPE_PlaySound( &tr, vecSrc, vecEnd, iBulletType );
				DecalGunshot( &tr, iBulletType );
				break;
			case BULLET_MONSTER_LARGEROUND:
				pEntity->TraceAttack( pevAttacker, gSkillData.monDmgLargeRound, vecDir, &tr, DMG_BULLET );
	...
	for( ULONG iShot = 1; iShot <= cShots; iShot++ )
	{
		...
		if( tr.flFraction != 1.0f )
		{
			...
			case BULLET_PLAYER_SMALLROUND:
				pEntity->TraceAttack( pevAttacker, gSkillData.plrDmgSmallRound, vecDir, &tr, DMG_BULLET );
				break;
			case BULLET_PLAYER_MEDIUMROUND:
				pEntity->TraceAttack( pevAttacker, gSkillData.plrDmgMediumRound, vecDir, &tr, DMG_BULLET );
				break;
			case BULLET_PLAYER_LARGEROUND:
				pEntity->TraceAttack( pevAttacker, gSkillData.plrDmgLargeRound, vecDir, &tr, DMG_BULLET );
				break;
